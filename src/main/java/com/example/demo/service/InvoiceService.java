...
public class InvoiceService {
    ...
    public InvoiceDTO createInvoice(InvoiceDTO dto) {
        ...
        Invoice invoice = Invoice.builder()
                ...
                .totalAmount(BigDecimal.ZERO) // เริ่มต้นที่ 0
    ...
    public InvoiceItemDTO addItem(InvoiceItemDTO itemDto) {
        ...
        return new InvoiceItemDTO(savedItem.getItemId(), savedItem.getDescription(), 
                                 savedItem.getAmount(), invoice.getInvoiceId());
    }

    //GET by ID
    public List<InvoiceDTO> getHistoryByPatient(Integer patientId) {
        return invoiceRepo.findByPatient_PatientId(patientId).stream()
    ...
    public List<InvoiceItemDTO> getInvoiceItems(Integer invoiceId) {
        return itemRepo.findByInvoice_InvoiceId(invoiceId).stream()
                .map(item -> new InvoiceItemDTO(item.getItemId(), item.getDescription(), 
                                               item.getAmount(), invoiceId))
                .collect(Collectors.toList());
    }
