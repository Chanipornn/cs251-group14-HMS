...
function displayRecords(list) {
    ...
    if (!list.length) {
        el.innerHTML = `<p class="no-data">ไม่พบข้อมูล</p>`;
        return;
    }
    ...
    el.innerHTML = list.map(r => `
        <div class="medical-card">
            ...
            <div class="card-top">
                <span><i class="fa-regular fa-calendar"></i> ${r.VisitDate}</span>
                <i class="fa-solid fa-chevron-right arrow"
                   onclick="goDetail('${r.RecordID}')"></i>
            </div>
            ...
            <div class="info-row"><span class="badge">อาการ</span>${r.Symptoms}</div>
            <div class="info-row"><span class="badge">วินิจฉัย</span>${r.Diagnosis}</div>
            <div class="info-row"><span class="badge">แผนก</span>${r.Dept_Name}</div>
            <div class="info-row"><span class="badge">แพทย์</span>${r.DoctorName}</div>
